    ...
    <MapContainer center={[city.coordinates.lat, city.coordinates.lng]} zoom={12} scrollWheelZoom className="h-full">
      ...
      {activeLayers.heat && city.hotspots.map((hotspot) => {
        ...
        const icon = new L.DivIcon({
          className: "",
          html: `<div style="height:18px;width:18px;border-radius:50%;background:${color};border:3px solid white;box-shadow:0 2px 10px rgba(32,36,38,.28)"></div>`,
          iconSize: [18, 18],
          iconAnchor: [9, 9]
        });
        ...
        return (
          <FeatureGroup key={hotspot.id}>
            <Circle
              ...
              radius={hotspot.radiusMeters ?? 400}
            ...
            >
              <Popup className="min-w-[200px]">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="text-xs">
                    <span className="font-semibold">Confidence:</span> {hotspot.confidenceScore ?? "Estimated"}
                  </div>
                  {hotspot.signals && (
                    <>
                      <div className="text-xs mt-1 pt-1 border-t">
                        <span className="font-semibold">Bldg Density:</span> {(hotspot.signals.buildingDensity * 100).toFixed(0)}%
                      </div>
                      <div className="text-xs">
                      ...
                      <div className="text-xs">
                        <span className="font-semibold">Water Prox:</span> {(hotspot.signals.waterProximity * 100).toFixed(0)}%
                      </div>
                    </>
                  )}
                  {(() => {
                    ...
                    return (
                      <div className="mt-2 pt-2 border-t">
                        <div className="text-xs">
                          <span className="font-semibold text-forest-700">Recommended:</span> {rec.action}
                        </div>
                        <div className="text-xs mt-1">
                          <span className="font-semibold text-forest-700">Benefit:</span> {rec.expectedBenefit}
                        </div>
